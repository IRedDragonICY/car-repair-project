...
struct dataLaporan {
  ...
  dataLaporan(string peminjam, string mobil, string plat, int penghasilan,
...
int dataLaporan::totalKeuntungan = 0;
...
public:
  laporanKeuangan() : head(nullptr), tail(nullptr) {}

  void tambahData(string peminjam, string mobil, string plat, int penghasilan,
                  int modal, int hari, string desc) {
    dataLaporan *newData =
        new dataLaporan(peminjam, mobil, plat, penghasilan, modal, hari, desc);
  ...
  int getTotalKeuntungan() { return dataLaporan::totalKeuntungan; }
};
