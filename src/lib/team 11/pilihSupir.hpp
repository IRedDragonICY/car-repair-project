...
public:
  Queue2();
  void enQueue2(string name, int duration, int price);
  void deQueue2();
  void displayQueue2();
  ~Queue2();
};

Queue2::Queue2() : front(NULL), rear(NULL) {}

void Queue2::enQueue2(string name, int duration, int price) {
  Queue2Node *newNode = new Queue2Node;
...
void Queue2::deQueue2() {
  if (front == NULL) {
    cout << "Antrian kosong.\n";
    return;
  }

  Queue2Node *temp = front;
  front = front->next;
...
void Queue2::displayQueue2() {
  Queue2Node *current = front;
  cout << "=========================================\n";
...
void Driver::sewaDriver() {
  ...
  rentalQueue2.enQueue2(customerName, rentalDuration, totalPrice);
}
