...
struct dataLaporanLinkedList {
  ...
  dataLaporanLinkedList *next;

  dataLaporanLinkedList(string peminjam, string mobil, string plat, int penghasilan,
      ...
      : namaPeminjam(peminjam), mobil(mobil), noPlat(plat),
        ...
        deskripsi(desc), next(nullptr) {
    totalKeuntungan += penghasilan;
  }
};

int dataLaporanLinkedList::totalKeuntungan = 0;

class laporanKeuanganLinkedList {
private:
  dataLaporanLinkedList *head;
  dataLaporanLinkedList *tail;

public:
  laporanKeuanganLinkedList() : head(nullptr), tail(nullptr) {}

  void tambahData(string peminjam, string mobil, string plat, int penghasilan,
                  int modal, int hari, string desc) {
    dataLaporanLinkedList *newData =
        new dataLaporanLinkedList(peminjam, mobil, plat, penghasilan, modal, hari, desc);
  ...
  void saveData() {
    ...
    dataLaporanLinkedList *current = head;
  ...
  void hapusData(int index) {
    ...
    if (index == 0) {
      dataLaporanLinkedList *temp = head;
      head = head->next;
      delete temp;
      return;
    }
    ...
    dataLaporanLinkedList *temp = current->next;
  ...
  int getTotalKeuntungan() { return dataLaporanLinkedList::totalKeuntungan; }
};

void menuLaporanLinkedlist(){
    laporanKeuanganLinkedList laporan;
  int choice;
