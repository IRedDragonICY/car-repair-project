...
Node1(Mobil mobil) : mobil(mobil), next(nullptr){}
};
...
public:
  ...
  Node1* find(string merk);
  void prosesPengembalian();
  RentalList() : head(nullptr){}
};

void RentalList::add(Mobil mobil){
  Node1* new_Node1 = new Node1(mobil);
  new_Node1->next = head;
  head->prev = new_Node1;
  head = new_Node1;
}

Node1* RentalList::find(string merk){
  Node1* current = head;
...
void RentalList::prosesPengembalian(){
  ...
  cin >> merk;
  Node1* Node1 = find(merk);
  if(Node1 != nullptr){
    cout << "Mobil " << Node1->mobil.merk << " berhasil dikembalikan" << endl;
    Node1->mobil.harga = "Rp.";
    Node1->mobil.jenis = " ";
    Node1->mobil.merk = " ";
    delete Node1;
  } else {
